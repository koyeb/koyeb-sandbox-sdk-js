...
Delete the pool (async server-side — `DELETING` status, fenced on active claims). The DELETE endpoint is rolling out; `delete()` ships ready and surfaces the API error until live.
